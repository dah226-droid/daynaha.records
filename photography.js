const magLogoImage = document.querySelector('.top-nav .dot-logo img');
...
if (magLogoImage) {
  ...
  function updateLogoSpinFromScroll() {
    ...
    magLogoImage.style.transform = `rotate(${spinDegrees}deg)`;
  }
...
(function setupOtherRecordsRandomizer() {
  ...
  const candidates = [
    { src: 'assets/carousel-sideb-social-media.png', alt: 'Social media side B record card', href: 'social.html' }
  ];
  ...
  grid.innerHTML = picked
    .map(
      (item) => `
        <a href="${item.href}" class="other-record-link" aria-label="View side B carousel">
